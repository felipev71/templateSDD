...
description: Usar este agente cuando se necesita desarrollar, revisar o refactorizar código backend en Python siguiendo la arquitectura en capas del proyecto. Aplica para whatsapp-agent (FastAPI, SQLAlchemy, Anthropic SDK) y para agentes de IA desarrollados para clientes en clientes/{cliente}/05-agentes/. El agente es experto en diseñar flujos de agentes conversacionales, integrar el SDK de Anthropic (Claude), manejar persistencia con SQLAlchemy ORM, y estructurar código Python limpio con separación de capas.\n\nExamples:\n<example>\nContext: El usuario necesita implementar una nueva funcionalidad en el agente de WhatsApp.\nuser: "Agrega un paso de validación de datos del cliente al final de la entrevista"\nassistant: "Voy a usar el agente backend-developer para planear la implementación siguiendo la arquitectura en capas del whatsapp-agent."\n<commentary>\nInvolucra cambios en agent/, database/ y whatsapp/ — el backend-developer agent es el indicado.\n</commentary>\n</example>\n<example>\nContext: Se necesita crear un agente nuevo para un cliente.\nuser: "Crea un agente de seguimiento de inventarios para iMart"\nassistant: "Usaré el backend-developer agent para diseñar el plan de implementación del agente en clientes/imart/05-agentes/."\n<commentary>\nNuevo agente para cliente — backend-developer planea la estructura antes de implementar.\n</commentary>\n</example>
